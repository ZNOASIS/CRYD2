...
def valid_crop_resize(data_numpy,valid_frame_num,p_interval,window):
    ...
    if len(p_interval) == 1:
        ...
        cropped_length = np.minimum(np.maximum(int(np.floor(valid_size*p)),300), valid_size)# constraint cropped_length lower bound as 64
...
def valid_crop_uniform(data_numpy, valid_frame_num, p_interval, window):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    begin = 0
    end = valid_frame_num
